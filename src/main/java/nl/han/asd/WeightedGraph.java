...
public class WeightedGraph {
    ...
    public WeightedGraph(List<List<List<Object>>> verbindingslijstGewogen) {
        adjList = new ArrayList<>();
        for (List<List<Object>> nodeEdges : verbindingslijstGewogen) {
            List<Edge> edges = new ArrayList<>();
            for (List<Object> edgeData : nodeEdges) {
                if (edgeData.size() >= 2 && edgeData.get(0) instanceof Number && edgeData.get(1) instanceof Number) {
    ...
    public void addEdge(int src, int dest, int weight) {
        ...
        if (adjList == null || adjList.isEmpty() || !isValidVertex(src) || !isValidVertex(dest)) {
            System.out.println("Invalid operation or vertex index.");
            return;
        }

        if (adjList.get(src).stream().noneMatch(edge -> edge.destination == dest)) {
            adjList.get(src).add(new Edge(dest, weight)); // Add only the directed edge src -> dest
        }
    }
